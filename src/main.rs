...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let gateways_cfg = if cfg.gateways.is_empty() {
        vec![GatewayConfig {
            ...
            allow_headers: cfg.allow_headers.clone(),
            sni: None,
    ...
    for g in gateways_cfg {
        let cert = g
            .cert
            .clone()
            .or_else(|| cli.cert.clone())
            .ok_or_else(|| ProxErr::Other("missing certificate".into()))?;
        let key = g
            .key
            .clone()
            .or_else(|| cli.key.clone())
            .ok_or_else(|| ProxErr::Other("missing key".into()))?;
        ...
        let allow_headers = if !g.allow_headers.is_empty() {
            g.allow_headers.clone()
        } else {
            cli.allow_header.clone()
        };
