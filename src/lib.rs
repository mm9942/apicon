...
use pingora::lb::{LoadBalancer, Backend, Backends};
...
pub struct Cli {
    ...
    /// TLS cert / key / CA
    #[arg(short = 'c', long = "cert", default_value = "/home/mm29942/projects/keys/live/api.mm29942.com/fullchain.pem")]
    pub cert: PathBuf,
    #[arg(short = 'k', long = "key",  default_value = "/home/mm29942/projects/keys/live/api.mm29942.com/privkey.pem")]
    pub key:  PathBuf,
    #[arg(long = "ca-root")]
...
impl ProxyHttp for LB {
    ...
    async fn request_filter(
        ...
        if sess.req_header().method == "OPTIONS" {
            ...
            if let Some(origin) = sess.req_header().headers.get("Origin").and_then(|o| {
                ...
                hdr.insert_header("Access-Control-Allow-Origin", origin).unwrap();
            }
            hdr.insert_header("Access-Control-Allow-Methods", "GET, POST, PUT, PATCH, DELETE, OPTIONS").unwrap();
            hdr.insert_header("Access-Control-Allow-Headers", "Content-Type, Authorization, X-Requested-With").unwrap();
            hdr.insert_header("Access-Control-Max-Age", "86400").unwrap();
    ...
    async fn response_filter(
        ...
        resp.insert_header("Access-Control-Expose-Headers", "Content-Length, Content-Type").unwrap();
        Ok(())
    }
}
...
impl Gateway {
    ...
    pub fn new(
        ...
        let mut tls = TlsSettings::intermediate(cert.to_str().unwrap(), key.to_str().unwrap()).expect("tls init");
        tls.enable_h2();
        ...
        let lb_service = LB { lb: lb.clone(), origins, endpoints };
...
pub fn resolve_ids(u: Option<&String>, g: Option<&String>) -> ProxResult<(Option<nix::unistd::Uid>, Option<nix::unistd::Gid>)> {
...
pub fn init_tracing(level: &str, json: bool, file: Option<&Path>) -> Option<tracing_appender::non_blocking::WorkerGuard> {
    ...
    if let Some(path) = file {
        ...
        if json {
            fmt::Subscriber::builder().with_env_filter(filter).with_writer(writer).json().init();
        } else {
            fmt::Subscriber::builder().with_env_filter(filter).with_writer(writer).init();
        }
        Some(guard)
    } else {
        if json {
            fmt::Subscriber::builder().with_env_filter(filter).json().init();
        } else {
...
pub fn systemctl_action(action: &str, names: &[String]) -> ProxResult<()> {
    ...
    for name in names {
        let out = Command::new("systemctl").arg(action).arg(name).output().map_err(ProxErr::from)?;
        print!("{}", String::from_utf8_lossy(&out.stdout));
        if !out.status.success() {
            eprintln!("{}", String::from_utf8_lossy(&out.stderr));
            return Err(ProxErr::Other(format!("systemctl {} failed for {}", action, name)));
        }
    }
    Ok(())
}

pub fn create_service(cli: &crate::Cli, names: &[String], workingdir: &Option<PathBuf>) -> ProxResult<()> {
    use std::io::Write;
    use std::process::Command;
    use std::fs::OpenOptions;
    ...
    if !args.iter().any(|a| a == "-d" || a == "--daemon") { args.push("-d".into()); }
    let base = format!("[Unit]\nDescription=apicon proxy\nAfter=network.target\n\n[Service]\nExecStart={} {}\n", exe.display(), args.join(" "));    let base = if let Some(wd) = workingdir { format!("{}WorkingDirectory={}\n", base, wd.display()) } else { base };
    for name in names {
        let mut service = base.to_owned();
        if let Some(u) = &cli.user { service.push_str(&format!("User={}\n", u)); }
        if let Some(g) = &cli.group { service.push_str(&format!("Group={}\n", g)); }
        ...
        let mut f = OpenOptions::new().create(true).write(true).truncate(true).open(path).map_err(ProxErr::from)?;
        f.write_all(service.as_bytes()).map_err(ProxErr::from)?;
        Command::new("systemctl").arg("daemon-reload").status().map_err(ProxErr::from)?;
        Command::new("systemctl").args(["enable", "--now", name]).status().map_err(ProxErr::from)?;
    }
    Ok(())
}
...
pub fn generate_mtls(out: &Path) -> ProxResult<()> {
    ...
    Command::new("openssl")
        .args(["req","-new","-newkey","rsa:2048","-nodes","-x509","-subj","/CN=apicon-test-ca","-days","1","-keyout",ca_key.to_str().unwrap(),"-out",ca_cert.to_str().unwrap()])
        .status().map_err(ProxErr::from)?;
    let srv_key = out.join("server.key");
    let srv_csr = out.join("server.csr");
    Command::new("openssl")
        .args(["req","-new","-newkey","rsa:2048","-nodes","-subj","/CN=localhost","-keyout",srv_key.to_str().unwrap(),"-out",srv_csr.to_str().unwrap()])
        .status().map_err(ProxErr::from)?;
    let srv_cert = out.join("server.pem");
    Command::new("openssl")
        .args(["x509","-req","-days","1","-in",srv_csr.to_str().unwrap(),"-CA",ca_cert.to_str().unwrap(),"-CAkey",ca_key.to_str().unwrap(),"-CAcreateserial","-out",srv_cert.to_str().unwrap()])
        .status().map_err(ProxErr::from)?;
    let cli_key = out.join("client.key");
    let cli_csr = out.join("client.csr");
    Command::new("openssl")
        .args(["req","-new","-newkey","rsa:2048","-nodes","-subj","/CN=client","-keyout",cli_key.to_str().unwrap(),"-out",cli_csr.to_str().unwrap()])
        .status().map_err(ProxErr::from)?;
    let cli_cert = out.join("client.pem");
    Command::new("openssl")
        .args(["x509","-req","-days","1","-in",cli_csr.to_str().unwrap(),"-CA",ca_cert.to_str().unwrap(),"-CAkey",ca_key.to_str().unwrap(),"-CAcreateserial","-out",cli_cert.to_str().unwrap()])
        .status().map_err(ProxErr::from)?;
