...
pub struct Cli {
    ...
    /// Allowed CORS headers (repeatable)
    #[arg(long = "allow-header", action = ArgAction::Append)]
...
pub struct GatewayConfig {
    ...
    pub allow_origin: Vec<String>,
    #[serde(default, rename = "allow_headers")]
...
impl ProxyHttp for LB {
    ...
    async fn request_filter(
        ...
        if sess.req_header().method == "OPTIONS" {
            ...
            .unwrap();
            hdr.insert_header("Access-Control-Allow-Headers", &self.headers.join(", "))
                .unwrap();
